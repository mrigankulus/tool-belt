// ---- Why o NPM for this?
// ---- copy/paste from https://github.com/neoziro/angular-draganddrop
// ---- This project isn't being maintained. The creator says that the npm
// ---- name is up for grabs. For drag n drop, we should be using dragula.
// ---- Was having trouble getting dragula to work for the permissions implementation,
// ---- so this seems like a pretty harmless addition. I'm sure that dragula will do
// ---- the job here...just need a little more tinkering.
// ---- If it needs to be said...todo: :)
// ------------------------------------------------------------------------------------

/*! Angular draganddrop v0.2.2 | (c) 2013 Greg Bergé | License MIT */

appModule.factory('draggableCurrentlyDragging', function () {
    return {
        currentlyDraggingData: {}
    }
});
 ...
 * - "draggable-data" Data attached to the dragged element, data are serialized in JSON.
     ...
     */
     ...
     function draggableDirective(draggableCurrentlyDragging) {
      return {
        restrict: 'A',
      ...
      domElement.addEventListener('dragstart', function (e) {
        ...
        draggableCurrentlyDragging.currentlyDraggingData = JSON.parse(jsonData);
